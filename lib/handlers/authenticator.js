...
const signinWithProvider = async ( providerMedia ) => {
	...
	// }
	await auth.signInWithPopup(provider).then(loginUser => 
		addUserToDatabaseAndStore(loginUser.user.displayName, loginUser.user.uid, loginUser.user.email, loginUser.user.photoURL)
	)
}
export const addUserToDatabaseAndStore = async (displayName, uid, email, profileImage) =>{
