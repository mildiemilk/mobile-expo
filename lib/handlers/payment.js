...
const SEND_EMAIL_TRANSACTION_DETAIL_PATH = 'send-email/transaction-detail'
const SEND_EMAIL_TRANSACTION_RESULT_PATH = 'send-email/transaction-result'
...
export const createPayment = async (transaction, refno) => {
	...
	await sendEmailSeller(transaction)
	return await sendEmailSponsor(transaction)
}

export const savePaymentImage = async (image, transaction) => {
	store.dispatch(savePaymentImagePending())
	...
	if(verifyTransaction(transaction)){
		...
		.then( async (snapshot) => {
			...
			await db.ref().update(updates)
			store.dispatch(addDateAndImage(updateTransaction.timestamp, updateTransaction.paymentImage))
			store.dispatch(savePaymentImageSuccess())
			await sendEmailBuyer(transaction.email, 'สถานะการจ่ายเงิน: รอดำเนินการ')
			await sendEmailSeller(transaction)
			await sendEmailSponsor(transaction)
		})
	} else {
...
export const approveBankTransferTransaction = async (transactionId,{sellerId, sponsorId, buyerId, price, comissionCash}) => {
	...
	const email = await getEmailByTransactionId(transactionId)
	await sendEmailBuyer(email, 'สถานะการจ่ายเงิน: ได้รับการตรวจสอบแล้ว')
}
...
const sendEmailSeller = async (transaction) => {
	console.log('send email seller', transaction)
	const { address1, province, postalCode, name, productName, price, comissionCash, sellerId } = transaction
	const email = await getEmailByUserId(sellerId)
	console.log('email seller', email)
	const detail = {
		transportDetail: address1 + ' ' + province + ' ' + postalCode,
		name,
		productName,
		comission: price * 0.9 - comissionCash
	}
	const data = {
		email,
		detail
	}
	return await callApi(SEND_EMAIL_TRANSACTION_DETAIL_PATH, data)
}

const sendEmailSponsor = async (transaction) => {
	console.log('send email sponsor', transaction)
	const { address1, province, postalCode, name, productName, price, comissionCash, sponsorId } = transaction
	const email = await getEmailByUserId(sponsorId)
	console.log('email sponsor', email)
	const detail = {
		transportDetail: address1 + ' ' + province + ' ' + postalCode,
		name,
		productName,
		comission: comissionCash
	}
	const data = {
		email,
		detail
...
export const sendEmailBuyer = async (email, text) => {
	const data = {
		email,
		text
	}
	console.log('send email buyer', data)
	return await callApi(SEND_EMAIL_TRANSACTION_RESULT_PATH, data)
}
...
const getEmailByTransactionId = async (transactionId) => {
	let db = await loadFirebase('database')
	const email = await db.ref().child(`transactions/${transactionId}/email`)
	let value
	await email.once('value', response => value = response.val())
	return value
}