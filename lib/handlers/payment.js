	...
	axios.post(
		...
		if(response.status === 200 ){
			...
			await db.ref().update(updates)
				.then(() => {
					const { sellerId, sponsorId, buyerId, price, comissionCash } = updates['/transactions/' + key]
...
export const savePaymentImage = async (image, transaction) => {
	...
	if(verifyTransaction(transaction)){
		await storage
			...
			.then( () => ({...transaction,timestamp:Date().toString}))
			.then( () => updates['/transactions/'+ key] = JSON.parse(JSON.stringify({...transaction, paymentStatus: 'pending' })))
		await db.ref().update(updates)
	} else {
...
const updateUserWallet = async (sellerUid, sponsorUid, price, comissionCash) => {
	let db = await loadFirebase('database')
