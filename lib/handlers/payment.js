    ...
    await store.dispatch(addPaymentAction({
      ...
      await db.ref().update(updates)
        .then(() => {
          ...
          Router.push('/')
          const { sellerId, sponsorId, buyerId, price, comissionPc, comissionCash } = updates['/transactions/' + key]
          updateUserTransaction(sellerId, sponsorId, buyerId, key)
          updateUserWallet(sellerId, sponsorId, price, comissionPc, comissionCash)
        })
		}
	})
}
...
const updateUserWallet = async (sellerUid, sponsorUid, price, comissionPc, comissionCash) => {
  let db = await loadFirebase('database')
  ...
  const sellerComission = price * 0.9 * (100 - comissionPc)/100 - comissionCash

  await sponsorWallet.once('value', response => sponsorWalletBalance = response.val())
  const sponsorComission = price * 0.9 * comissionPc/100 + comissionCash
