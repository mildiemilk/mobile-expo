...
import { addPayment as addPaymentAction, addTimeStamp as addTimeStampAction, transactionApproved } from '../actions/transaction'
...
export const createToken = async ({ cardNumber, securityCode, expiryMonth, expiryYear, name }) => {
	const response = await axios({
		...
		auth: {
			username: process.env.OMISE_PUBLIC_KEY
		}
	})
...
export const createPayment = async (amount, card, transaction, type, acquirer) => {
	let db = await loadFirebase('database')
	let token = await createToken(card)
	await store.dispatch(savePaymentPending())
	axios.post(
		'/api/charges', {
		...
		if(response.status === 200 ){
			...
			await db.ref().update(updates)
				.then(() => {
					...
					store.dispatch(savePaymentSuccess())
				})
		}
	})
	.catch((e) => {
...
export const createPaymentInternetBanking = async (amount, currency, offsite, transaction) => {
	let db = await loadFirebase('database')
	let key = await db.ref().child('transactions').push().key
	let updates = {}
	const return_uri = `http://${window.location.hostname}${window.location.port?`:${window.location.port}`:''}/payment/${key}`
	updates['/transactions/'+ key] = JSON.parse(JSON.stringify(transaction))
	await db.ref().update(updates)
	await axios.post('/api/charges/internet-banking', { 
		amount, 
		currency, 
		offsite, 
		return_uri
	...
	.then( async response => {
		...
		updates['/transactions/'+ key] = JSON.parse(JSON.stringify(transaction))
		await db.ref().update(updates)
		.then(() => window.location.href = response.data.authorize_uri)
	})
	.catch((e) => {
		console.log(e)
		alert('payment failed')
...
export const savePaymentInternetBanking = async key => {
	...
	if(transaction) {
		const url = '/api/charges/internet-banking/' + transaction.payment.paymentDetail.id
		const response = await axios.get(url)
		transaction.payment.paymentDetail = response.data
		if(response.data.status==='successful'){
			...
			db.ref().child(`transactions/${key}`).remove()
			alert('payment failed')
			Router.push('/')
		}
	} else {
...
export const savePaymentImage = async image => {
	...
	if(verifyTransaction()){
		await storage
			...
			.put(image[0])
			.then( snapshot => store.dispatch(savePaymentImageSuccess(snapshot.downloadURL)))
			.then( () => store.dispatch(addTimeStampAction()))
			.then( () => updates['/transactions/'+ key] = JSON.parse(JSON.stringify({...store.getState().transaction, paymentStatus: 'pending' })))
		await db.ref().update(updates)
	} else {
...
export const approveBankTransferTransaction = async (transactionId,{sellerId, sponsorId, buyerId, price, comissionCash, comissionPercent}) => {
		...
		await updateUserWallet(sellerId, sponsorId, price, comissionPercent, comissionCash)
