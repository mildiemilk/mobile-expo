...
export const createPayment = async (amount, card, transactions, type, acquirer) => {
  console.log('create Payment', amount, card, transactions)
      ...
      paymentDetail: response.data
		...
		if(response.status === 200 ){
			Object.keys(transactions).forEach(async  transaction => {
				...
				let ownerWallet 
				let sellerWallet 
				await db.ref(`wallets/${ownerUid}/wallet`).on('value', response=> {ownerWallet = response.val()})
				await db.ref(`wallets/${sellerUid}/wallet`).on('value', response => {sellerWallet = response.val()})
				let updates = {}
				if(transactions[transaction].ownerUid === transactions[transaction].sellerUid){
					updates['/wallets/'+ownerUid +'/wallet'] = parseFloat(ownerWallet) + parseFloat(comissionOwner) + parseFloat(comissionSeller)
				} else {
					updates['/wallets/'+ownerUid +'/wallet'] = parseFloat(ownerWallet) + parseFloat(comissionOwner)
					updates['/wallets/'+sellerUid +'/wallet'] = parseFloat(sellerWallet) + parseFloat(comissionSeller)
				}
				updates['/transactions/'+newTransactionId] = transactions[transaction]
				updates['/wallets/'+sellerUid+'/comission-as-seller/'+newTransactionId] = comissionSeller
				updates['/wallets/'+ownerUid+'/comission-as-owner/'+newTransactionId] = comissionOwner
				console.log('updastes', updates)
