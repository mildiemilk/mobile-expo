...
import { setNewChatroomSuccess, updateChatroomMessageSuccess } from '../actions/chat';
...
export const updateChatroom = async (currentChatroom, chatroomId) => {
    const db = await loadFirebase('database')
    const nextChatroom = {
        ...currentChatroom,
        chats: [
    ...
    updates[`/chatrooms/${key}`] = nextChatroom
    db.ref().update(updates)
    store.dispatch(updateChatroomMessageSuccess(nextChatroom))
}

export const onChangeChatroom = async (key) => {
    const db = await loadFirebase('database')    
    await db
    .ref(`chatroom/${key}/chats`)
    .on("value", chatroom => store.dispatch(setChats(chatroom.val())))
}