...
export const setMembers = async(membership) => {
	...
	await database
		...
		// .on("value", members => store.dispatch(setMembersSuccess(convertObjectToArray(members.val()))))
...
const setMemberUserInfo = async(member) => {
	...
	await database
		...
		.on("value", user => store.dispatch(setMemberSuccess({
			...member,
			...user.val()[member.userUid]
		})))
}

const setMemberPermission = async(uid, permission)=>{
	
} 

export const getAlianceProducts = async(aliances) => {
	// get all members products 
}
