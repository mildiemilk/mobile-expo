...
import { 
	...
	setNewMembershipPasswordPending,
	setNewMembershipPasswordSuccess } from '../actions/memberships'
import md5 from 'md5'
...
const extractMembers = membershipByEmail => membershipByEmail? Object.keys(membershipByEmail).map( key => membershipByEmail[key].membership):null
...
export const setNewMembershipPassword = async (membership, password) => {
	store.dispatch(setNewMembershipPasswordPending())
	const database = await loadFirebase('database')
	const updates = {}
	updates['/memberships/'+membership+'/password'] = md5(password)
	await database.ref().update(updates).then(async( snap )=> {
		await store.dispatch(setNewMembershipPasswordSuccess())
	})
}