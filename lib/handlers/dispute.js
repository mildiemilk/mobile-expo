...
export const saveDispute = async (email, dispute, callback) => {
	try {
		...
		await sendEmailDispute(email, dispute, 'กำลังดำเนินการ')
		callback('success')
	} catch (error) {
...
export const updateStatusDisputes = async (key,index) => {
	try {
		...
		const dispute = await getDisputeByDisputeId(key)
		await sendEmailDispute(email, dispute, 'ถอนเงินสำเร็จ')
...
const sendEmailDispute = async (email, dispute, status) => {
	const { DateAndTime, amount } = dispute
	const detail = {
		date: DateAndTime,
		amount,
		status
...
const getDisputeByDisputeId = async (disputeId) => {
	let db = await loadFirebase('database')
	const dispute = await db.ref().child(`disputes/${disputeId}`)
	let value
	await dispute.once('value', response => value = response.val())
	return value
}