...
import { 
	...
	setProductImageByKey,
	setProductImageByKeyPending
...
export const setProductImage = async (key, image) => {
	store.dispatch(setProductImageByKeyPending(key))
...
export const getProductFromTransactionByUserUid = async (buyerId, email) => {
	...
	const value = await database
		...
		.once('value')
		.then(snapshot => snapshot.val())
		const result = value ? Object.values(value).filter(item=> item.email === email) : []
		return result
}
...
export const getProductToSponsorTable = async (buyerId, email) =>{
	...
	const allData = sponsor?transaction.concat(sponsor):null
	const unique = allData?[...new Set(allData.map(item => item.productId))]:null
	unique?unique.map(element => getProductSponsorFromID(element)):null
}
...
export const deleteProduct = async (productId) => {
	const database =await loadFirebase('database')
	database.ref('products').child(productId).remove()
	alert('สินค้าถูกลบเรียบร้อยแล้ว')
	Router.push('/profile')
}

export const setInitialProductValue = async(product) => {
	store.dispatch(change('product', 'productName' , product.productName))
	store.dispatch(change('product', 'brandName' , product.brandName))
	store.dispatch(change('product', 'comissionCash' , product.comissionCash))
	store.dispatch(change('product', 'price' , product.price))
	store.dispatch(change('product', 'comissionCash' , product.comissionCash))
	store.dispatch(change('product', 'stock' , product.stock))
	store.dispatch(change('product', productDescription))
}
