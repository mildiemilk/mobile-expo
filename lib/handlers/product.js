...
import { 
	...
	setProductImage as setProductSubImage
...
export const setProductImage = async (image) => {
	const storage = await loadFirebase('storage')
	await storage.ref('images').child(`${Date.now()}.jpg`).put(image[0]).then( snapshot => store.dispatch(setProductImageSuccess(snapshot.downloadURL)))
}

export const setSubImage = async (key, image) => {
	const storage = await loadFirebase('storage')
	await storage.ref('images').child(`${Date.now()}.jpg`).put(image[0]).then( snapshot => store.dispatch(setProductSubImage(snapshot.downloadURL, key)))
}
