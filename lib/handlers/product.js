...
export const setMembershipProduct = async ( userUid ) => {
	...
	database.ref('products')
		...
		.once('value') 
		.then(products => 
			filterOnlyMemberProducts(products.val())
		)
}

const filterOnlyMemberProducts = products =>
	Object.keys(products).map( key => products[key].isMembership? store.dispatch(setMemberProductSuccess(key,products[key])):null)
