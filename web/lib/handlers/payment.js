...
import { savePaymentImageSuccess, savePaymentImagePending } from '../actions/payment'
...
export const savePaymentImage = async (image, transaction) => {
	...
	if(verifyTransaction(transaction)){
		...
		.then( async (snapshot) => {
			...
			await sendEmailBuyer(updateTransaction, 'รอดำเนินการ')
		})
	} else {
...
export const approveBankTransferTransaction = async (transactionId,{sellerId, sponsorId, buyerId, price, comissionCash}) => {
	...
	await sendEmailBuyer(transaction, 'ได้รับการตรวจสอบแล้ว')
}
...
export const sendEmailSeller = async (transaction) => {
...
export const sendEmailSponsor = async (transaction) => {
...
export const sendEmailBuyer = async (transaction, status) => {
	const { productName, price, email, timestamp } = transaction
	const detail = {
		date: timestamp,
		productName,
		price,
...
const getTransactionByTransactionId = async (transactionId) => {
	let db = await loadFirebase('database')
	const transaction = await db.ref().child(`transactions/${transactionId}`)
	let value
	await transaction.once('value', response => value = response.val())
	return value
}
